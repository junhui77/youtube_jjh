...
//		System.out.println("------------------------------MemberController.java || /loginCheck  || 컨트롤러 완료~~~~~ sesMemAuth(권한):" + memberInfo.getMemAuth()  );
	...
	public String registerMember( @RequestParam("upload") MultipartFile upload,MultipartHttpServletRequest request, MemberVo member, Model model) throws IllegalStateException, IOException {
		
		log.info("*******************/imgInsert************************");
		System.out.println("------------------------------MemberController.java || /registerMember  || 컨트롤러 완료~~~~~ registerImg():" + member +"," + upload);
		...
		member.setMemIp(ip);
		memberService.registerMember(member, upload);// 기본 회원정보 등록
...
//		model.addAttribute("memIp",memIp);
	...
	@RequestMapping(value="/procUpdateMyPage", method=RequestMethod.POST)
	public String updateMemberMyPage(@RequestParam("upload") MultipartFile upload ,HttpServletRequest request, MemberVo member) {
		memberService.updateMember(member, upload);
	...
	public String updateMember(@RequestParam("upload") MultipartFile upload ,HttpServletRequest request, MemberVo member) {
		memberService.updateMember(member, upload);
		return "redirect:memberList";
	}
	...
	public @ResponseBody HashMap<String,Object> addFollow(@PathVariable String writer, HttpSession session){
		...
		if(findFollow == 0) {
			int memberFollow = memberService.memberFollow(followVo);// 구독 여부 확인
			int followCount = memberService.memberFollowCount(writer);
			System.out.println("-------------------------HomeController.java || /addFollow || 진입 완료 memberFollow : " + memberFollow);
			System.out.println("-------------------------HomeController.java || /addFollow || 진입 완료 memberFollowCount : " + followCount);
	...
	//회원 구독 취소
	@RequestMapping(value="/removeMemberFollow/{writer}")
	public @ResponseBody HashMap<String, Object> deleteFollow(@PathVariable String writer, HttpSession session){
		...
		System.out.println("-------------------------HomeController.java || /deleteFollow || 진입 완료 memId : " + memId);
		System.out.println("-------------------------HomeController.java || /deleteFollow || 진입 완료 writer : " + writer);
		HashMap<String, Object> resMap = new HashMap<String, Object>();
		int findFollow = memberService.memberFindFollow(followVo);
		
		if(findFollow == 1) {
			memberService.memberRemoveFollow(followVo);
			int followCount = memberService.memberFollowCount(writer);
