...
//		model.addAttribute("boardList", list);
	...
	public String boardDetail(@RequestParam("num") Integer num,@RequestParam("writer") String toMemId ,Model model, HttpSession session,Criteria criteria) { //@RequestParam("writer")String toMemId,
	...
	//	ReplyVo replyVo = new ReplyVo();
		...
		System.out.println("-------------------------HomeController.java || /boardDetail || start~~~~~ followCount :" + memberService.memberFollowCount(toMemId));
		...
		System.out.println("-------------------------HomeController.java || /boardDetail || start~~~~~ replyCount :" + replyCount);
	...
	//체크하여 게시글 삭제
		...
		public @ResponseBody HashMap<String,Object> boardLike(@PathVariable Integer num, HttpSession session){
			...
			if(findLike == 0) { // 좋아요 안 누른 상태
				boardService.boardLike(boardLikeVo); // 좋아요 등록
		...
		public @ResponseBody HashMap<String, Object> deleteLike(@PathVariable Integer num, HttpSession session){
			...
			if(boardFindLike == 1) {
				boardService.boardRemoveLike(boardLikeVo); //좋아요 삭제
				Integer boardLikeCount = boardService.boardLikeCount(num); //좋아요 개수
...
//		@ResponseBody
//		@RequestMapping(value = "/boards/{num}/replies", method = RequestMethod.GET)
//		public HashMap<String, Object> list(HashMap<String, Object> num, @RequestParam(defaultValue = "10") int perPage, @RequestParam(defaultValue = "1") int page, @RequestParam(defaultValue = "5") int pageSize) {
//			
//			logger.info("================댓글 리스트 출력 Controller================");
//			
//			return replyService.retrieveReplyList(num, perPage, page, pageSize);
//		}
		...
		public @ResponseBody HashMap<String,Object> regReply(@RequestBody ReplyVo replyVo, @PathVariable("num") Integer num, HttpSession session, Model model, MemberVo memberVo, Criteria criteria ){	
			...
			//댓글 조회
			//List<ReplyVo> replyVoList = replyService.retrieveReplyList(num, perPage, page, pageSize);
		...
		public @ResponseBody HashMap<String, Object> deleteReply(@RequestBody ReplyVo replyVo,@PathVariable("num") Integer num, @PathVariable("replyIdx") Integer replyId, HttpSession session, Criteria criteria){
		...
		public @ResponseBody HashMap<String, Object> modifyReply(@RequestBody ReplyVo replyVo,
				@PathVariable("num") Integer num, @PathVariable("replyIdx") Integer replyIdx, HttpSession session, Criteria criteria) {
